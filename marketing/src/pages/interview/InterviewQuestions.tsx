...
const questions: Question[] = [
  // Основы
  { q: 'Что такое Marketing Mix (4P)? Приведите пример для SaaS.', a: 'Product — CRM-система для малого бизнеса. Price — $29/мес (freemium → paid). Place — сайт + AppStore + партнёрские интеграции. Promotion — контент-маркетинг, SEO, вебинары. 4P расширились в 7P для услуг: + People (люди), Process (процессы), Physical Evidence (физическое подтверждение).', level: 'junior', topic: 'Основы' },
  ...
  { q: 'Разница между маркетинговой и продуктовой стратегией?', a: 'Продуктовая: что мы делаем и для кого (roadmap, фичи, PMF). Маркетинговая: как мы об этом рассказываем (каналы, месседжи, позиционирование). Они пересекаются: GTM (Go-To-Market — вывод на рынок) — мост между продуктом и рынком. Маркетолог не создаёт продукт, но формирует его восприятие.', level: 'middle', topic: 'Основы' },
  ...
  { q: 'Как выбрать каналы для B2B vs B2C?', a: 'B2B: LinkedIn, вебинары, white papers, email-nurturing, conference, ABM (Account-Based Marketing — маркетинг ключевых клиентов). Длинный цикл, несколько ЛПР. B2C: Instagram, TikTok, YouTube, influence-маркетинг (маркетинг влияния), programmatic (програматик — автоматизированная закупка рекламы). Короткий цикл, эмоциональные решения. B2B = educate, B2C = entertain + convert.', level: 'middle', topic: 'Digital' },
  { q: 'Как построить омниканальную стратегию?', a: 'Омниканальность ≠ мультиканальность. Мульти: много каналов изолированно. Омни: единый customer experience через все каналы. Шаги: (1) single customer view (единый профиль клиента), CDP (Customer Data Platform — платформа данных клиентов), (2) согласованные месседжи, (3) attribution модель, (4) персонализация по стадии воронки. Ключ: данные + координация.', level: 'senior', topic: 'Digital' },

  // Аналитика
  { q: 'Какие основные метрики маркетинга?', a: 'Верхнеуровневые: CAC, LTV, ROMI (Return on Marketing Investment — возврат маркетинговых инвестиций). Канальные: CTR, CPC, CPM, CPA. Контентные: охват, engagement rate (коэффициент вовлечённости), share of voice (доля упоминаний). Email: open rate, click rate, unsubscribe. Важно отслеживать unit economics: CAC payback period (срок окупаемости).', level: 'junior', topic: 'Аналитика' },
  ...
  { q: 'Как считать ROI маркетинговой кампании с attribution?', a: 'ROMI = (Revenue - Marketing Cost) / Marketing Cost × 100%. Проблема: multi-touch journey. Решение: (1) Инкрементальные тесты (geo-lift (гео-тест), holdout groups (контрольные группы)), (2) Media Mix Modeling / MMM (моделирование медиамикса), (3) Data-driven attribution (атрибуция на основе данных) в GA4. Правильный ROI = incremental revenue (инкрементальный доход) / cost, не просто last-click revenue.', level: 'senior', topic: 'Аналитика' },
  ...
  { q: 'Как измерить силу бренда?', a: 'Количественно: Brand Awareness (aided/unaided recall — узнавание с подсказкой / без), NPS, brand search volume (объём брендовых запросов), Share of Voice. Качественно: brand associations (ассоциации с брендом), brand equity (капитал бренда). Методики: BrandZ, Interbrand ranking. Для стартапов: direct traffic growth + branded search + NPS trend.', level: 'middle', topic: 'Бренд' },
  ...
  { q: 'Как оптимизировать CPA при работе с рекламой?', a: '1) Тестировать креативы (3-5 вариантов), 2) A/B тесты лендингов, 3) Оптимизировать по воронке: CPC → CTR → conv rate, 4) Bid strategy (стратегия ставок): target CPA (целевая стоимость конверсии) в Google, cost cap (ограничение стоимости) в Meta, 5) Ретаргетинг (дешевле, чем cold), 6) Negative keywords (минус-слова), placement exclusions (исключение площадок). Правило: если CPA > target 20%, пауза и анализ.', level: 'middle', topic: 'Performance' },
  { q: 'Как построить предиктивную модель маркетингового бюджета?', a: 'Подходы: (1) % от выручки (benchmark: 5-15%), (2) Target CAC × target customers, (3) Marginal ROAS (предельная рентабельность рекламы) — увеличиваем бюджет пока incremental ROAS > 1, (4) Media Mix Modeling — econometric model показывает оптимальное распределение. Ключ: diminishing returns (убывающая отдача) — каждый следующий доллар менее эффективен.', level: 'senior', topic: 'Performance' },
]
