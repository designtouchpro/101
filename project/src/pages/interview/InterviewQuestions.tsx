...
const questions: Question[] = [
  ...
  { q: 'Как масштабировать Agile? SAFe vs LeSS.', a: 'SAFe (Scaled Agile Framework — масштабируемый Agile): PI Planning (планирование инкремента), ART (Agile Release Train — поезд релизов), решение для больших организаций. Тяжёлый фреймворк. LeSS (Large-Scale Scrum): один Product Owner (владелец продукта) на несколько команд, минимум добавленных ролей. Spotify Model: Squads (отряды), Tribes (племена), Chapters (секции), Guilds (гильдии). Выбор зависит от размера и культуры компании.', level: 'senior', topic: 'Методологии' },
  ...
  { q: 'Key person risk — как митигировать?', a: 'Риск зависимости от одного специалиста. Митигация: (1) Bus factor (фактор автобуса — сколько людей может выбыть без потерь) ≥ 2 для критичных систем, (2) Документация + knowledge sharing, (3) Парное программирование / code review, (4) Ротация задач, (5) Cross-functional training. Метрика: кол-во модулей с bus factor = 1.', level: 'middle', topic: 'Риски' },
  ...
  { q: 'Как управлять конфликтом между командами?', a: 'Модель Thomas-Kilmann: (1) Competing (соперничество) — когда срочно, (2) Collaborating (сотрудничество) — когда важны обе стороны, (3) Compromising (компромисс) — средний путь, (4) Avoiding (уклонение) — когда не стоит свеч, (5) Accommodating (уступка) — сохранить отношения. PM ищет win-win (выигрыш для обеих сторон) через сотрудничество. Эскалация — крайняя мера.', level: 'senior', topic: 'Коммуникация' },
  ...
  { q: 'EVM: как считать SPI и CPI?', a: 'EVM (Earned Value Management — управление освоенным объёмом): PV (Planned Value) — плановая стоимость, EV (Earned Value) — освоенная стоимость, AC (Actual Cost) — фактические затраты. SPI = EV/PV (Schedule Performance Index — индекс сроков: >1 опережаем). CPI = EV/AC (Cost Performance Index — индекс стоимости: >1 экономим). EAC = BAC/CPI — прогноз итоговой стоимости. Если CPI < 0.8 — проект в красной зоне.', level: 'senior', topic: 'Метрики' },
]
